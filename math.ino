// shift 1 jam 07:00 - 18:00
// shift 2 jam 19:00 - 06:00

void changeShift() {
  if (Hour > 6 and Hour < 18) {       // Change shift siang
    shift = 1;
  }
  else if (Hour > 18 and Hour < 24) {  // Change shift malam
    shift = 2;
  }
  ...
  if (Hour == 17 and String(readFile(SPIFFS, "/reset.txt")) == "0") {
    xshift = shift;
  ...
  if (Hour == 18) {
    if (String(readFile(SPIFFS, "/reset.txt")) == "1") {
      backup_changeshift();
    }
  }

  if (Hour == 19 and String(readFile(SPIFFS, "/reset.txt")) == "0") {
    writeFile(SPIFFS, "/reset.txt", "1");
    delay(500);
  }

  if (Hour == 5 and String(readFile(SPIFFS, "/reset.txt")) == "0") {
    xshift = shift;
    writeFile(SPIFFS, "/reset.txt", "1");
    delay(500);
  }
  ...
  if (Hour == 7 and String(readFile(SPIFFS, "/reset.txt")) == "0") {
    writeFile(SPIFFS, "/reset.txt", "1");
    delay(500);
  }
  ...
  if (xshift != shift) {
    if (hm == 1) {
      logging(status, String(xshift), timer_hmOn, timer_hmOff, String(selisih_hm), true, "normal");
      delay(500);
      xshift = shift;
    }
    if (idle == 1) {
      logging(status, String(xshift), timer_idleOn, timer_idleOff, String(selisih_idle), true, "normal");
      delay(500);
      xshift = shift;
    }
    if (standby == 1) {
      logging(status, String(xshift), timer_standbyOn, timer_standbyOff, String(selisih_standby), true, "normal");
      delay(500);
      xshift = shift;
    }
  }
...
void parsing_backup() {
  String parse_backupOff = ":" + String(times);

  int batasbackup1Off = parse_backupOff.indexOf(':');
  int batasbackup2Off = parse_backupOff.indexOf(':', batasbackup1Off + 1);
  int batasbackup3Off = parse_backupOff.indexOf(':', batasbackup2Off + 1);
  int batasbackup4Off = parse_backupOff.indexOf(':', batasbackup3Off + 1);

  jam_backupOff = parse_backupOff.substring(batasbackup1Off + 1, batasbackup2Off);
  mnt_backupOff = parse_backupOff.substring(batasbackup2Off + 1, batasbackup3Off);
  dtk_backupOff = parse_backupOff.substring(batasbackup3Off + 1, batasbackup4Off);

  String parse_tglbackupOff = "-" + String(dates);

  int batasbackup1tglOff = parse_tglbackupOff.indexOf('-');
  int batasbackup2tglOff = parse_tglbackupOff.indexOf('-', batasbackup1tglOff + 1);
  int batasbackup3tglOff = parse_tglbackupOff.indexOf('-', batasbackup2tglOff + 1);
  int batasbackup4tglOff = parse_tglbackupOff.indexOf('-', batasbackup3tglOff + 1);

  tahun_backupOff = parse_tglbackupOff.substring(batasbackup1tglOff + 1, batasbackup2tglOff);
  bulan_backupOff = parse_tglbackupOff.substring(batasbackup2tglOff + 1, batasbackup3tglOff);
  hari_backupOff = parse_tglbackupOff.substring(batasbackup3tglOff + 1, batasbackup4tglOff);
}
